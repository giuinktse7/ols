...
if "%1" == "test" (
    ...
    odin build src\ -show-timings  -microarch:native  -collection:src=src  -out:%build_dir%\ols.exe -o:minimal  -no-bounds-check -use-separate-modules -debug  -extra-linker-flags:"/STACK:4000000,2000000" -define:VERSION=%version%-debug
) else (
    if not exist "%build_dir%" mkdir "%build_dir%"
    odin build src\ -show-timings -microarch:native -collection:src=src -out:%build_dir%\ols.exe -o:speed  -no-bounds-check  -extra-linker-flags:"/STACK:4000000,2000000" -define:VERSION=%version%
)
